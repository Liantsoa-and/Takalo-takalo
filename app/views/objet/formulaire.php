    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="objetForm" method="POST" enctype="multipart/form-data" novalidate
                        action="<?= $isEdit ? $base . '/objet/' . $objet['id'] . '/update' : $base . '/objet/create' ?>">

                        <!-- Titre de l'objet -->
                        <div class="mb-3">
                            <label for="libelle" class="form-label"><span class="text-danger">*</span> Titre de
                                l'objet</label>
                            <input type="text" class="form-control" id="libelle" name="libelle"
                                value="<?= $isEdit ? htmlspecialchars($objet['libelle']) : '' ?>"
                                placeholder="Ex: Veste en cuir" required minlength="3" maxlength="255">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="5"
                                placeholder="Décrivez votre objet en détail..." required
                                minlength="10"><?= $isEdit ? htmlspecialchars($objet['description']) : '' ?></textarea>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="category_id" class="form-label"><span class="text-danger">*</span>
                                    Catégorie</label>
                                <select class="form-select" id="category_id" name="category_id" required>
                                    <option value="">Sélectionnez une catégorie</option>
                                    <?php foreach ($categories as $category): ?>
                                        <option value="<?= $category['id'] ?>" <?= $isEdit && $objet['category_id'] == $category['id'] ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($category['libelle']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <!-- Prix estimatif -->
                            <div class="col-md-6 mb-3">
                                <label for="prix_estimatif" class="form-label"><span class="text-danger">*</span> Prix
                                    estimatif (€)</label>
                                <input type="number" class="form-control" id="prix_estimatif" name="prix_estimatif"
                                    value="<?= $isEdit ? htmlspecialchars($objet['prix_estimatif']) : '' ?>"
                                    placeholder="0.00" step="0.01" min="0" max="9999.99" required>
                                <div class="invalid-feedback">Le prix est obligatoire et doit être un nombre positif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control" id="photos" name="photos[]" multiple
                                accept="image/*">
                            <small class="form-text text-muted">Vous pouvez télécharger plusieurs photos (JPG, PNG,
                                etc.)</small>
                        </div>
                                    ...
                                    <?php foreach ($objet['photos'] as $photo): ?>
                                        <div class="col-md-3 position-relative">
                                            <div class="position-relative">
                                                <img src="<?= htmlspecialchars($photo['url']) ?>" class="img-fluid rounded"
                                                    alt="Photo de l'objet">
                                                <a href="<?= $base ?>/photo/<?= $photo['id'] ?>/delete"
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary"><?= $isEdit ? 'Modifier' : 'Ajouter' ?>
                                l'objet</button>
