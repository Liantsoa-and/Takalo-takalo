...
<head>
    ...
    <title>Objets disponibles - Takalo-takalo</title>
    ...
    <div class="container mt-5">
        ...
        <div class="filter-section">
            ...
            <?php if ($currentCategory): ?>
                <div class="mt-2">
                    <a href="<?= $base ?>/objets_publics" class="btn btn-sm btn-outline-secondary">Réinitialiser le filtre</a>
                </div>
            <?php endif; ?>
        </div>
                ...
                <?php foreach ($objets as $objet): ?>
                    <div class="col">
                        <div class="card object-card">
                            <?php if ($objet['main_photo']): ?>
                                <img src="<?= htmlspecialchars($objet['main_photo']) ?>" 
                                     class="card-img-top object-img" 
                                     alt="<?= htmlspecialchars($objet['libelle']) ?>">
                            <?php else: ?>
                                <div class="card-img-top object-img bg-secondary d-flex align-items-center justify-content-center">
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="badge bg-success"><?= number_format($objet['prix_estimatif'], 0, ',', ' ') ?> Ar</span>
                                    <small class="text-muted">
                                        Par <?= htmlspecialchars($objet['owner_name']) ?>
                                    </small>
                                </div>
                            </div>
                            <div class="card-footer">
                                <a href="<?= $base ?>/objet/<?= $objet['id'] ?>" class="btn btn-primary w-100">
                                    Voir détails
                                </a>
                            </div>
                        </div>
            ...
            <nav aria-label="Navigation des pages">
                <ul class="pagination justify-content-center">
                    ...
                    <li class="page-item <?= $currentPage <= 1 ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=<?= $currentPage - 1 ?><?= $currentCategory ? '&category_id=' . $currentCategory : '' ?>">
                            Précédent
                        </a>
                    </li>
                    ...
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                        <li class="page-item <?= $i == $currentPage ? 'active' : '' ?>">
                            <a class="page-link" href="?page=<?= $i ?><?= $currentCategory ? '&category_id=' . $currentCategory : '' ?>">
                                <?= $i ?>
                            </a>
                        </li>
                    <?php endfor; ?>
