...
<main class="col-12 col-md-9 col-lg-10 py-4">
    ...
    <div class="filter-section mb-4">
        <form id="searchForm-<?= htmlspecialchars($scope) ?>" class="search-form" data-scope="<?= htmlspecialchars($scope) ?>" data-target="#objects-list-public" data-base="<?= $base ?>">
            <div class="row g-3">
                <div class="col-md-5">
                    <div class="input-group">
                        ...
                        <input type="search" name="q" class="form-control border-start-0" placeholder="Rechercher un objet..." value="<?= htmlspecialchars($q ?? '') ?>">
                    </div>
                </div>
            ...
            <div class="filter-chips">
                <?php if (!empty($q)): ?>
                    <span class="filter-chip active">
                        ...
                        <span class="remove-chip" onclick="document.querySelector('input[name=q]').value=''; this.closest('form').submit();">×</span>
                    </span>
                <?php endif; ?>
                ...
                <?php if (!empty($currentCategory)): ?>
                    <span class="filter-chip active">
                        ...
                        <span class="remove-chip" onclick="document.querySelector('select[name=category_id]').value=''; this.closest('form').submit();">×</span>
                    </span>
                <?php endif; ?>
                        ...
                        <div class="card object-card">
                            ...
                            <div class="position-relative">
                                <?php if (!empty($objet['main_photo'])): ?>
                                    <img src="<?= htmlspecialchars($objet['main_photo']) ?>" 
                                         class="card-img-top object-img" 
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-actions">
                                    ...
                                    <a href="<?= $base ?>/objet/<?= $objet['id'] ?>/history" 
                                       class="icon-action icon-history" 
                                       title="Historique">
                        ...
                        <div class="object-details">
                            ...
                            <div class="object-footer">
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="<?= $base ?>/objet/<?= $objet['id'] ?>/history" class="btn btn-sm btn-outline-secondary">
        ...
        <nav aria-label="Navigation des pages" class="mt-4">
            <ul class="pagination justify-content-center">
                ...
                <li class="page-item <?= ($currentPage ?? 1) <= 1 ? 'disabled' : '' ?>">
                    <a class="page-link" href="?page=<?= ($currentPage ?? 1) - 1 ?><?= !empty($currentCategory) ? '&category_id=' . $currentCategory : '' ?><?= !empty($q) ? '&q=' . urlencode($q) : '' ?>">
                ...
                if ($start > 1): ?>
                    <li class="page-item">
                        <a class="page-link" href="?page=1<?= !empty($currentCategory) ? '&category_id=' . $currentCategory : '' ?><?= !empty($q) ? '&q=' . urlencode($q) : '' ?>">1</a>
                    </li>
                    <?php if ($start > 2): ?>
                ...
                <?php for ($i = $start; $i <= $end; $i++): ?>
                    <li class="page-item <?= $i == ($currentPage ?? 1) ? 'active' : '' ?>">
                        <a class="page-link" href="?page=<?= $i ?><?= !empty($currentCategory) ? '&category_id=' . $currentCategory : '' ?><?= !empty($q) ? '&q=' . urlencode($q) : '' ?>">
                            <?= $i ?>
                        </a>
                    </li>
                <?php endfor; ?>
                ...
                <?php if ($end < ($totalPages ?? 1)): ?>
                    ...
                    <?php endif; ?>
                    <li class="page-item">
                        <a class="page-link" href="?page=<?= $totalPages ?><?= !empty($currentCategory) ? '&category_id=' . $currentCategory : '' ?><?= !empty($q) ? '&q=' . urlencode($q) : '' ?>"><?= $totalPages ?></a>
                    </li>
                <?php endif; ?>
